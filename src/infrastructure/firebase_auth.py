...
class FirebaseAuth:
    ...
    def verificar_id_token(self, id_token: str) -> str:
        """Valida o idToken e retorna o uid. Lança ErroAutenticacao se inválido."""
        try:
            decodificado = auth.verify_id_token(id_token)
        except Exception as exc:
            raise ErroAutenticacao("idToken inválido ou expirado.") from exc

        uid = decodificado.get("uid")
        if not uid:
            raise ErroAutenticacao("Token sem uid.")
        return uid
